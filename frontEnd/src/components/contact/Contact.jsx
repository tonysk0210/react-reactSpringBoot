...
export default function Contact() {
  ...
  const contactInfo = useLoaderData(); // useLoaderData 是 React Router 提供的一個 hook，用於在組件中獲取 loader 函數返回的數據。在這個組件中，當 contactLoader 函數被調用並返回數據後，這些數據會被 useLoaderData 捕獲並存儲在 contactInfo 變量中。這些數據可以在組件中用來顯示聯絡信息或者其他相關信息給用戶。contactInfo 改變時會觸發組件重新渲染，從而更新 UI 上顯示的聯絡信息。
