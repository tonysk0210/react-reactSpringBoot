...
export default function Footer() {
  return (
    <>
      {/* 使用 cssModule 的 className; 注意：className={cssModule.footer} 的寫法 */}
      <footer className={`${cssModule.footer} ${DarkModeClass}`}>
        Built with{" "}
        <FontAwesomeIcon
          icon={faHeart}
          className={cssModule["footer-icon"]}
          // 使用 cssModule 的 className; 注意：因為 class name 中有連字號（-），所以要用 className={cssModule["footer-icon"]} 的寫法
          aria-hidden="true"
        />
        by{" "}
      ...
      {/* 使用 styled-components 套件。注意 props 的寫法：$primary 是一個自定義的 props，$ 是一個慣例，表示這個 props 只給 styled-components 使用，不傳到 HTML */}
