...
一個以 **React 19 + Spring Boot 3** 建構的完整電商系統，涵蓋商品瀏覽、購物車、Stripe 線上刷卡付款、訂單管理與後台管理功能。

本專案以展示全端開發能力為目標，著重於安全性設計、前後端分離架構、狀態管理策略與生產環境可用性（Production-readiness）。
...
- 查看所有待處理訂單，執行確認或取消操作
- 查看所有未處理客戶留言，標記為已關閉
- 透過 Swagger UI 瀏覽完整 API 文件

### 平台層
- JWT 無狀態身份驗證（1 小時效期，401 後自動導向登入頁）
- Cookie-based CSRF 防護，覆蓋所有狀態變更請求
- Caffeine 記憶體快取，商品列表 TTL 30 分鐘，降低資料庫負載
- JPA Auditing，所有資料表自動記錄建立者、修改者及時間戳
